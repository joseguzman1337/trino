        ...
        implements ConnectorPageSinkProvider
{
    ...
    public ConnectorPageSink createPageSink(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorOutputTableHandle outputTableHandle, Optional<ConnectorTableCredentials> credentials, ConnectorPageSinkId pageSinkId)
    {
        ...
        return createPageSink(session, tableHandle, extractFileIoProperties(credentials));
    }
        ...
        switch (executeHandle.procedureId()) {
            case OPTIMIZE:
                ...
                return new IcebergPageSink(
                        ...
                        fileWriterFactory,
                        pageIndexerFactory,
                        fileSystemFactory.create(session.getIdentity(), extractFileIoProperties(credentials)),
    ...
    private static Map<String, String> extractFileIoProperties(Optional<ConnectorTableCredentials> credentials)
    {
        if (credentials.isPresent() && credentials.get() instanceof IcebergTableCredentials(Map<String, String> fileIoProperties)) {
    ...
    public ConnectorMergeSink createMergeSink(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorMergeTableHandle mergeHandle, Optional<ConnectorTableCredentials> credentials, ConnectorPageSinkId pageSinkId)
    {
        ...
        Map<String, String> fileIoProperties = extractFileIoProperties(credentials);
        LocationProvider locationProvider = getLocationProvider(tableHandle.name(), tableHandle.outputPath(), tableHandle.storageProperties());
        Schema schema = SchemaParser.fromJson(tableHandle.schemaAsJson());
